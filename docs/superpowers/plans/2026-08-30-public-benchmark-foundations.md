...
- Starting point for amendment-dependent Tasks 2–5 remains governance-only successor `6930b6e18b11d50a4df5b5fd18d37207e891a28c`, which synchronizes the approved prior amendment without changing normative behavior. Task 1 remains already-committed baseline-authorized history. Tasks 10–12 start from this governance-only successor after its future full SHA is recorded at handoff as `PLAN_BASE_SHA`; the plan source does not embed that future commit SHA. Keep remaining implementation on an isolated `codex/` branch or worktree based on the applicable exact commit.
